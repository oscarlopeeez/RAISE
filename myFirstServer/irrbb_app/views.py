...
class UploadContractsView(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            return self.form_invalid(form)
        # Forzar uso de la última curva BCE al importar contratos
...
class DetailView(LoginRequiredMixin, TemplateView):
        ...
        try:
            ...
            if contributions:
                ...
                for c in contributions:
                    ...
                    # store contract-level meta so we can compute carrying and duration for snapshot
                    try:
                        target[producto]['contracts'].append({'nominal': float(c.get('nominal', 0.0) or 0.0), 'fecha_vencimiento': c.get('fecha_vencimiento')})
                    except Exception:
                        pass
                # compute carrying_amount and duration per product from snapshot contributions
                from datetime import date
                try:
            ...
            try:
                ...
                pass
            # Usar la curva BCE más reciente obligatoriamente
            market_curve = MarketCurve.objects.first()
            if not market_curve:
    ...
    def get_context_data(self, **kwargs):
        try:
            ...
            raise Http404('Resultado de balance no encontrado')
        # Usar la última curva de mercado (BCE). Si no existe, indicar error.
        ...
        # provide a predictable static logo path for the banco (static files should contain matching images)
        bank_logo = None
        try:
            if resultado.banco:
...
class CurveView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        # Construir puntos a partir de la curva BCE más reciente
        market = MarketCurve.objects.first()
        points = []
        if market:
            curve_df = build_curve_from_market(market)
            ...
            # si no hay curva, puntos queda vacío (la plantilla mostrará mensaje)
            points = []
        ...
        if market:
            ...
            # No comparison against a hardcoded default curve — the base curve is
            # constructed from market data. We keep market_diff empty to avoid
            # misleading zero diffs when base==market.
            market_diff = []
        svg_points = []
        market_svg_points = []
        ...
        # build observations list explicitly so we can ensure a fallback
        observations = []
        # Only keep age-based observation; comparisons vs a removed default curve

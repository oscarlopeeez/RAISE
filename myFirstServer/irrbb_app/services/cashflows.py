...
from .nmds import effective_maturity, is_nmd
...
def build_cashflows(contract, curve_df, valuation_date=None):
    ...
    fecha_vencimiento_efectiva = effective_maturity(contract, valuation_date)
    payment_dates = generate_payment_dates(contract.fecha_inicio, fecha_vencimiento_efectiva, contract.frecuencia_cupon)
    ...
    if freq <= 0:
        freq = 1
    period_rate = effective_rate(contract, curve_df) / freq
    ...
    for i, pay_date in enumerate(payment_dates, start=1):
        ...
        if nom_rest_after < 0:
            nom_rest_after = 0
        rows.append({'contract_id': contract.id, 'payment_date': pay_date.date(), 'period_start': last_date.date(), 'period_end': pay_date.date(), 'year_fraction': year_frac, 'year': t, 'rest_start': nom_rest * sign, 'interest': interest * sign, 'principal': principal * sign, 'cashflow': (interest + principal) * sign, 'rate_per_period': period_rate, 'is_floating': 1 if contract.tipo_interes == contract.VARIABLE else 0, 'activo_pasivo': contract.activo_pasivo})
